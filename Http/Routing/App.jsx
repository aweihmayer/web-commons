...
class App extends React.Component {
    ...
    render() {
        ...
        if (code >= 300 || code < 200 && Routes.error) {
            ...
            else route = new Route({ view: () => <p>Implement the route "error.{code}" or "error.default" for a custom error page.</p> });
        }

        if (route === null) {
            if (Routes.error && Routes.error.hasOwnProperty(404)) route = Routes.error[404];
            else route = new Route({ view: () => <p>Implement the route "error.404" for a custom error page.</p> });
        }
